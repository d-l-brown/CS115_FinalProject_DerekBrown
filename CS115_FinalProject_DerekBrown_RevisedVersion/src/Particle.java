...
public class Particle
{
	...
	Particle(Random r, double speed, double radius, double m)
	{
		...
		this.x_velocity = speed*Math.pow(-1,r.nextInt(1));
		this.y_velocity = speed*Math.pow(-1,r.nextInt(1));
		this.z_velocity = speed*Math.pow(-1,r.nextInt(1));
	}
	...
	{
		return Math.sqrt(
				Math.pow(x_velocity,2)
				+Math.pow(y_velocity,2)
				+Math.pow(z_velocity,2)
				);
	}
	
	public double dot()
	{
		return x_position*x_velocity +y_position*y_velocity+z_position*z_velocity;
	}
}